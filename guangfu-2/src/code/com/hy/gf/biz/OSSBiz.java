...
public class OSSBiz {
	 ...
	 * @return
	 */
	public String[] uploadFilesData(MultipartHttpServletRequest request) {
	...
	/**
	 * 图片上传oss,不涉及后台，用户头像修改
	 * 
	 * @param request
	 * @return
	 * @throws IOException 
	 */
	public String userUploadFilesData(MultipartHttpServletRequest request) throws IOException {
		MultipartFile file = request.getFile("__avatar1");
		String realpath = request.getSession().getServletContext().getRealPath("/resources/images");
		String operateName = System.currentTimeMillis() + StringUtil.CreateRandomCode(8)+".jpg";
		String path = realpath+File.separator +operateName;
