...
public class CreateVueEdit {
	...
	public static void createEntity(String packagePath, String tableName, String domainObjectName) {
		...
		String readFileByLines = FileUtil.readFileByLines("src/main/java/template/edit.vue");
...
//		System.out.println(content);
		...
		String path = System.getProperty("user.dir") + StrUtil.codePath + "vueEdit/";
...
//		System.out.println("resPath=" + resPath);
	...
	private static String createDiv(String content) {
		...
		for (int i = 0; i < colnames.length; i++) {
			...
			if (colnames[i].contains("Dtm")) {
				replacement.append("                  <div class='form-group col-xs-6'> <label for=''></label> <input value='{{data.title}}' type='text' class='form-control' id='' placeholder=''></div>\n");
			}else{
				replacement.append("                  <div class='form-group col-xs-6'> <label for=''></label> <input value='{{data."+colnames[i]+"]}}' type='text' class='form-control' id='' placeholder='请输入'></div>\n");
			}
		}
		content=content.replace(XLabel.fordiv, replacement);
