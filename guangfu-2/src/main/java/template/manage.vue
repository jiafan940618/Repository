            ...
            <div class="box-body">
              <table id="example1" class="table table-bordered table-striped">
                ...
                <tr v-for='item in list | orderBy sort'>
                  ...
                  <td>{{item.id}}</td>
                  <td><a href="#" v-link="{path:'/product/edit/'+item.id}">{{item.$:model.name}}</a></td>
                  $:fortd
                </tr>
                </tbody>
                <tfoot>
...
export default {
  ...
  methods: {
    ...
    init () {
      this.$http({url: '$:model.name/selectByExample', method: 'GET', data: { index: this.page }}).then(function (response) {
  ...
  events: {
    delete () {
      ...
      this.$http({url: '$:model.name/deleteBatch', method: 'GET', data: { list: delList }}).then(function (response) {
        var data = response.data
        if (data.code !== 200) {
          window.alert('fail')
        } else {
